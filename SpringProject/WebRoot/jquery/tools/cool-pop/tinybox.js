...
TINY.box=function(){
	var p,m,b,fn,ic,iu,iw,ih,ia,f=0,content;
	var containerId;
	return{
		show:function(c,u,w,h,a,t,alpha){
			if(c.charAt(0)=='#'){
				containerId=c.substr(1);
				c=T$(containerId).innerHTML;
				T$(containerId).innerHTML="";
			}
			content=c;
			if(!f){
			...
			ic=c; iu=u; iw=w; ih=h; ia=a; this.alpha(m,1,g,3);
		...
		hide:function(){
			//恢复div内容
			if(containerId){
				T$(containerId).innerHTML=content;
			}
